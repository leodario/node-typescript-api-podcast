...
[Doc HTTP](https://nodejs.org/docs/latest/api/http.html)

### Iniciado Projeto sem fazer perguntas
```
npm init -y
```
### Instalando as Dependências Typescript em Modo de Desenvolvimento (tsup converte em typescript)
```
npm i typescript tsx tsup -D
```
### Chamando o executável do código binário do typescript (Criando o arquivo de configuração no typescript tsconfig.json)
```
npx tsc --init
```
